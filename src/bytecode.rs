...
impl ByteCode {
    pub(crate) fn from(code: &[u8], pool: &[Rc<ConstantPoolEntry>]) -> Result<Self, ParseError> {
        let bytecode = Self {
...
fn read_opcodes(code: &[u8], pool: &[Rc<ConstantPoolEntry>]) -> Result<Vec<(usize, Opcode)>, ParseError> {
